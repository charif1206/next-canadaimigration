...
export default function FormActions({ isPending, isSuccess, isValid }: FormActionsProps) {
  return (
    <div className="flex gap-4 pt-4">
      ...
      >
        {isPending ? (
          <span className="flex items-center justify-center gap-2">
            ...
            {ADD_ADMIN_MESSAGES.BUTTON.CREATING}
          </span>
        ) : isSuccess ? (
          <span className="flex items-center justify-center gap-2">
            ...
            {ADD_ADMIN_MESSAGES.BUTTON.CREATED}
          </span>
        ) : (
          ADD_ADMIN_MESSAGES.BUTTON.CREATE
        )}
      </button>
      ...
      >
        {ADD_ADMIN_MESSAGES.BUTTON.CANCEL}
      </Link>
    </div>
  );
}
